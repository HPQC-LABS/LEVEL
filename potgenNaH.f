c***********************************************************************
      SUBROUTINE POTGEN_NaH-XA(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
...
c  and centrifugal potential functions for the X and A states of NaH 
c  reported by Walji et al. [JCP 142, xxxx (2015)]. 
c----------------------------------------------------------------------
...
c  a value for parameters ISTATE @ lines 80-87  
c==============
c*** On INPUT:
c==============
...
ccc Define variables for the ISMX states of interest
     ...
     1 CMMA(9,ISMX),PARMA(NbMX,ISMX),XPARMA(NbMX,ISMX),U1infA(ISMX),
     2 U2infA(ISMX),T1INFA(ISMX),T2infA(ISMX),U1A(0:NBOB,ISMX),
     3 U2A(0:NBOB,ISMX),T1A(0:NBOB,ISMX),T2A(0:NBOB,ISMX)
...
ccc Store parameter values for the specific ISMX states of interest
     ...
     1  sVSR2A/-2,0/, IDSTTA/1,1/, NCMMA/3,0/, MMLRA/6,8,10,6*0, 9*0/, 
     2  MN1RA/23,23/, MN2RA/1,1/,PADA/6,3/, QADA/4,3/, QNAA/ 3,3/, 
     3  NU1A/-1,-1/, Nu2A/4,3/, Nt1A/-1,-1/, Nt2A/2,1/
      DATA DSCMA/15797.4d0,10041.0d0/, REQA/1.887023d0,3.1927d0/,
     3  RrefA/3.0d0,3.0d0/, rhoABA/0.69d0,0.d0/
...
c** and Finally the adiabatic BOB parameters
     ...
     g T2infA/0.d0,0.d0/,
c... and up to 11 expansion parameters for each of the ISMX states
...
c** Prepare Asen's Rlk array for later use in generating Spline fx. 
                  CALL Lkoef(Nbeta,XPARM,rKL,NbMX)
                  ENDIF
              IF(Rref.GT.0) THEN
                  WRITE(6,613) Rref
                ELSE
...
c... calculate Pashov cubic spline exponent coefficient  ...
                  BETA= 0.d0
                  DO m= 1,Nbeta
                      BETA= BETA + Scalc(ZQ,m,Nbeta,XPARM,rKL,NbMX)
     1                                                        *PARM(m)
                      ENDDO
                ENDIF
