...
c***********************************************************************
     ...
     2  dv2,ANS2,XDIF,VLIM,VMAXX,BMAX,Pi,Pi2,PNCN,PWCN,PP1,VDMV,ENEXT,
     3   V(NDP)
...
c
      ...
      PWCN= 2.d0
      IF(NCN.NE.2) PWCN= 2.d0*NCN/ABS(NCN- 2.d0)
      PNCN= ABS(NCN-2)/FLOAT(NCN+2)
      DGDVBP= DGDVB**PNCN
      PP1= 1.d0/pNCN + 1.d0
      I3= NDP
      IF(EO.GT.VLIM) THEN
...
c... here collect (v+1/2) and dv/dG integrals over outer well ....
   ...
   12 I3= II+1
      ...
      Y3= EINT- V(I3+1)
      CALL LEVQAD(Y1,Y2,Y3,RH,RT,ANS1,ANS2)
      VPH2= (VPH2 - ARG2 - 0.5d0*ARG3 + ANS2/RH)/Pi
...
c     RETURN
  600 FORMAT('Single well  ICOR=',I2,':  E(v=',i3,',J=',I3,')=',f10.2,
     1 '  v(SC)=',F8.3/24x,'dGdv=',f8.3,'   (vD-v)=',f8.4,
     2 '   E(next)=',1PD12.4)
  601 FORMAT('Single well  ICOR=',I2,':  E(v=',i3,',J=',I3,')=',
     1  1PD12.4,'  v(SC)=',0PF8.3/22x,'dGdv=',1PD10.3,0P,
     2 '   (vD-v)=',f8.3,'   E(next)=',1PD12.4)
