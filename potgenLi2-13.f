...
c  in the normal way with R.J. Le Roy's program LEVEL, once one inserts a
c  chosen value for  parameter ISTATE.  
c==============
c*** On INPUT:
c==============
...
c-----------------------------------------------------------------------
      ISTATE= 3
c-----------------------------------------------------------------------
ccc Specify parameter values for the two Li2 states of interest
      DATA PPARA/5,5,6,6/, MMLRA/6,8,10,0, 6,8,10,0, 3,0,6,8,3,-1,6,8/,
     1  NLRA/16,3,16,9/, NU1A/2,0,4,3/,QPARA/3,3,3,2/ 
      DATA DSCMA/8516.708d0,333.7795d0,9353.1795d0,7093.4926d0/,
     1  REQA/2.6729874d0,4.170006d0,3.1079288d0,3.065436d0/,
     2  RrefA/4.07d0,8.0d0,4.4d0,4.8d0/, 
c** specify the 4 C_m long-range coefficients for the 4 states
     4  CMMA/6.7190d6,1.12635d8,2.78694d9,0.d0, 6.7190d6,1.12635d8,
     5    2.78694d9,0.d0, 3.578352d5,0.3353246d0,1.00059d7,3.69965d8,
     6    3.576828d5,0.3353246d0,1.00059d7,3.69965d8/,
c** now specify 20 exponent parameters \beta_i for the 4 states
     7    PARMA/1.3904114D-01,-1.430265D+00,-1.499723D+00,-6.5696D-01,
     8  3.3156D-01,1.02298D+00,1.2038D+00,1.223D+00,3.122D+00,6.641D+00,
     9  3.71D-01,-1.217D+01,2.98D+00,2.86D+01,6.8D+00,-2.52D+01,
     a   -1.5D+01,3*0.D0,
     b   -5.16129D-01,-9.8D-02,1.133D-01,-2.51D-02, 16*0.d0,
     c   -1.757571385D+00,-1.0348756D+00,-1.811999D+00,-1.62322D+00,
     d  -1.44465D+00,1.23225D+00,2.96072D+00,2.5664D+00,1.9043D+00,
     e  1.6423D+01,2.0631D+01,-2.6932D+01,-5.55D+01,8.1D+00,4.56D+01,
     f  2.7D+00,-1.4D+01,3*0.d0,
     g   -1.819413208D+00,-4.729259D-01,-5.18639D-01,-1.6109D-01,
     h   -8.608D-01,3.933D+00,9.65D-01,-2.81D+00,-2.27D+00,1.2D+00,
     i   10*0.d0/
c** Specify 6 'adiabatic' BOB parameters for up to 4 states
      DATA U1A/2.21D-01,1.8D-01,5.4D-01,3*0.d0,
     1    6.9D-02,5*0.d0,
     2    1.249D+00,3.8D+00,-1.09D+00,3.9D+00,-6.4D+00,1*0.d0,
     3    1.596D+00,3.1D+00,-1.5D+00,-2.0D+00,2*0.d0/
      LNPT= 1
      IAN1= 3
      IAN2= 3
      IPOTL= 4
      ...
      rhoAB= -1.d0
      IF((ISTATE.EQ.1).OR.(ISTATE.EQ.2)) rhoAB= 0.54d0
      DSCM= DSCMA(ISTATE)
      REQ= REQA(ISTATE)
      Rref= RrefA(ISTATE)
c
...
c  For simplicity, first zero out all correction function coefficients
          ...
          DO  I= 0, NU1
              ...
              ENDDO
          U1INF= 0.d0
          IF(ISTATE.GE.3) U1INF= 1.2315155D+00
          U2INF= U1INF
c=======================================================================
