c***********************************************************************
c     SUBROUTINE POTGENBe2X(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
      SUBROUTINE POTGEN(LNPT,NPP,IAN1,IAN2,IMN1,IMN2,VLIM,XO,RM2,VV,
     1                                                        NCN,CNN)
c-----------------------------------------------------------------------
c         *******    Version of  14 November 2013    **********
c** Subroutine to generate the effective adiabatic potential energy 
c  function for the ground X(^2\Sigma^+) electronic state of Be2 using the 
c   MLR(6) potential function of: Meshkov et al.[J.Chem.Phys 140, xxx (2014).
c-----------------------------------------------------------------------
...
c*** On INPUT:
c==============
c*  input integer  MN1  specifies the mass number of the  first Be atom
c*  input integer  MN2  specifies the mass number of the  second Be atom
c   Since Be has only one stable isotope, one should set  MN1=MN2=9
c*  integers IAN1=IAN2=4  are the atomic numbers dentifying the chemical 
c           species
...
ccc Specify parameter values for the X{^2\Sigma^+} state of  MgH 
     ...
     1  NLRA/6/, NU1A/-1/, NU2A/-1/,  NT1A/-1/,NT2A/-1/
      DATA DSCMA/934.8d0/, REQA/2.445d0/, RrefA/3.4d0/
...
c** specify  up to 4 C_m long-range coefficients for each state
     2  CMMA/1.03130D+06,1.38060D+07,1.90580D+8,0.d0/
c** now specify up to 20 exponent parameters \beta_i for each state
     3 PARMA/5.5203D-01,-1.922D-01,-1.306D+00,6.65D-01,8.80D-01,
     4   -1.61D+00,-1.22D+00,13*0.d0/
