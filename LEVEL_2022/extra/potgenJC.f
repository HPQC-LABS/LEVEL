...
c=======================================================================
      IF(IPOTL.EQ.4) THEN
          IF(LNPT.GT.0) THEN
              ...
              CNN= CMM(1)
              ULRe= 0.d0
              IF((NCMM.GE.3).AND.(MMLR(2).EQ.0)) THEN
c** For special Aubert-Frecon Li2(A) {3,0,6,6} type case 
                  RM3= 1.d0/REQ**3
...
c** For the 'regular' simple inverse-power sum case.
     ...
     1                                    IVSR,IDSTT,KDER,DM,DMP,DMPP)
                  DO  J= 1,NCMM
                      ...
                      ENDDO
                ENDIF
ccc           BETA= (ULR/ULRe)*DEXP(-BETA*ZZ)
c!! JAC adjustment(s)
              BETA= (ULR/ULRe)*DEXP(-BETA*YPJC)
