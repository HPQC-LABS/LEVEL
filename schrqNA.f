c***********************************************************************
c***** R.J. Le Roy  subroutine SCHRQ, last updated 30 May 2005 *********
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
c!! Version adapted to do iterative 'Non-adiabatic'(?) correction for
c     paper with Tyuterev.  Never used since.!!
c=======================================================================
c                COPYRIGHT 2005  by  Robert J. Le Roy                  +
...
c----------------------------------------------------------------------
      SUBROUTINE SCHRQ(KV,JROT,EO,GAMA,VMAX,VLIM,VV,WF,BFCT,EEPS,RMIN,
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      IF(KVIN.LT.-10) THEN
          ...
          SPNEND= WF(NEND-1)
          ENDIF
c!! Modification for non-adiabatic kinetic energy correction
      FIX= 0
    4 CONTINUE
      S0= 0.d0
      S0= -4.5d-04
...
c  large, then shift inner starting point outward.
              ...
              IF((ITER.LE.1).AND.(IWR.NE.0)) THEN
                  ...
                  IF(GN.LE.Z0) WRITE(6,604) JROT,EO,NBEG,V(NBEG)/BFCT
                  ENDIF
  ...
  100 IF(IWR.NE.0) THEN
     ...
     1                  .AND.(RMIN.GT.0.d0)) WRITE(6,614) JROT,EO,RATIN
          IF((E.LT.DSOC).AND.(DABS(RATOUT).GT.RATST)) THEN
              ...
              ENDIF
          ENDIF
c!!!!!!!!!! Segment to determine self-consistent energy for Non-Ad corrn
      IF(DABS(S0).GT.0.d0) THEN
          IF(DABS(E-EBEF).GT.EPS) THEN
ccc           write(6,333) kv,jrot,FIX,eO
              FIX= FIX+1
  333 format('   v=',i2,'   J=',i3,'   E(fix=',i3,')=',f14.4)
              IF(FIX.LT.20) GOTO 4
              ENDIF 
          ENDIF
      KKV = 0
