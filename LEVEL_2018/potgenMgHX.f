c***********************************************************************
c     SUBROUTINE POTGENMgHX(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
      SUBROUTINE POTGEN(LNPT,NPP,IAN1,IAN2,IMN1,IMN2,VLIM,XO,RM2,VV,
     1                                                        NCN,CNN)
c-----------------------------------------------------------------------
c         **********    Version of  02 July 2013    **********
c** Subroutine to generate the effective adiabatic potential energy 
c  function for the ground X(^2\Sigma^+) electronic state of MgH and any 
c  the nine ^{24,25,26}Mg^{1,2,3}H isotopologues using the MLR potential
c   functions of: Henderson et al.[J.Chem.Phys-A (2013).
c-----------------------------------------------------------------------
...
c  in the normal way with R.J. Le Roy's program LEVEL. 
c==============
c*** On INPUT:
c==============
c*  input integer  MN1  specifies the mass number of the  Mg isotope
c*  input integer  MN2  specifies the mass number of the  H isotope
c*  integers IAN1 and IAN2  are the atomic numbers dentifying the chemical species
...
c=======================================================================
     ...
     1  PAD,QAD,PNA,QNA,NU1,NU2,NT1,NT2,NCMAX,PPAR,QPAR,NCN,NSR,NLR,
     2  NVARB,NPP,LNPT,GNS,GEL, NCMM,IVSR,LVSR,IDSTT,KDER,MM1, MMLR(9)
      CHARACTER*2 NAME1,NAME2
      REAL*8  A0,A1,A2,A3,ALFA,Asw,Rsw,BETA,BINF,B1,B2,BT,CSAV,U1INF,
     1 U2INF,T1INF,T2INF,YPAD,YQAD,YQADSM,YPNA,YQNA,YQNASM,ABUND,CNN,
     ...
     8 XO(NPP),VV(NPP),RM2(NPP), bTT(-1:2),cDS(-2:0),bDS(-2:0)
      SAVE IBOB,IPOTL,IORD,IORDD,PPAR,QPAR,PAD,QAD,PNA,QNA,NSR,
...
c** Define arrays for storing parameter for one or multiple states
      INTEGER NSTATES
c** For the MgH system, NSTATES=1 but there may be many isotopologues
      PARAMETER (NSTATES=1)
      INTEGER ISTATE,PPARA(NSTATES),QPARA(NSTATES),NLRA(NSTATES),
     1  NCMMA(NSTATES),MMLRA(4,NSTATES),NU1A(NSTATES),NU2A(NSTATES),
     2  PADA(NSTATES),QADA(NSTATES),NT1A(NSTATES),NT2A(NSTATES),
     3  PNAA(NSTATES),QNAA(NSTATES)
      REAL*8 DSCMA(NSTATES),REQA(NSTATES),RrefA(NSTATES),
     1 rhoABA(NSTATES),CMMA(4,NSTATES),PARMA(20,NSTATES),
     2 U1A(15,NSTATES),U2A(15,NSTATES),T1A(10,NSTATES),T2A(10,NSTATES)
...
c---------------------------------------------------------------------------
ccc Specify parameter values for the X{^2\Sigma^+} state of  MgH 
      DATA PPARA/5/, QPARA/4/,MMLRA/6,8,10,0/
     1  NLRA/12/, NU1A/3/, NU2A/13/,  NT1A/-1/,NT2A/7/
      DATA DSCMA/11104.25d0/, REQA/1.7296854d0/, RrefA/2.74d0/
c** specify the value of  rhoAB  for each state
     2  rhoABA/0.81d0/,
c** specify the number of Cm coefficients for each state
     3  NCMMA/3/,
c** specify  up to 4 C_m long-range coefficients for each state
     2  CMMA/2.7755d+05,3.4549d+06,4.614d+07,0.d0/
c** now specify up to 20 exponent parameters \beta_i for each state
     3  PARMA/1.17047546d0,1.0801579d0,2.6732971d0,2.48359d0,0.74013d0,
     4   0.19147d0,0.60383d0,-2.4873d0,-7.6653d0,-5.731d0,2.839d0,
     5   6.054d0,2.4d0, 7*0.d0/
c** Specify 'adiabatic correction parameter types {for each state}
      DATA PADA/6/,QADA/4/
c** Specify up to 15 'adiabatic' BOB parameters for the first atom (Mg)
c ...  UiINF value at end of each set.
      DATA U1A/1.31d0,4.18d0,2.67d0,4.0d0,0.0d0,10*0.d0/
c** Specify up to 15 'adiabatic' BOB parameters for the second atom (H)
     1     U2A/-15.183d0,37.524d0,-11.607d0,32.2d0,60.57d0,229.5d0,
     2      -1561.d0,-2013.d0,12530.d0,5360.d0,-49740.d0,10500.d0,
     3       71100.d0,-47000.d0,0.0d0/
c** Specify centrifugal BOB correction parameter types {for each state}
      DATA PNAA/4/,QNAA/4/
c** Specify up to 10 centrifugal BOB parameters for the first atom (Mg)
c ...  TiINF value at end of each set.
      DATA T1A/10*0.D0/
c** Specify up to 10 centrifugal BOB parameters for the second atom (H)
     2     T2A/0.0d0,7.266d-04,2.7d-04,9.11d-04,2.97d-03,-1.9d-03,
     2        -7.2d-03,2.3d-02,0.0d0, 0.d0/
      LNPT= 1
c*** Specify the identity of the molecular specues
      IAN1= 12
      IAN2= 1
c*** Specify the identity of the reference isotopologue
      MN1R= 24
      MN2R= 1
c** specify the nature of thw radual potential  IPOTL= 4  for an MLR
      IPOTL= 4
      IBOB= 1
c** Assume Douketis-type damping with  s= -1
      IVSR= -2
      IDSTT= 1
c*** have only one state for MgH so ISTATE= 1
      ISTATE= 1
c*** Note that rhoAB is the same for the X and A states
      rhoAB=rhoABA(ISTATE)
      NCMM= NCMMA(ISTATE)
      DSCM= DSCMA(ISTATE)
      REQ= REQA(ISTATE)
...
c-----------------------------------------------------------------------
          ...
          QNA= QNAA(ISTATE)
c-----------------------------------------------------------------------
cc            READ(5,*) MN1R, MN2R, PAD, QAD, NU1, NU2, PNA, QNA,NT1,NT2
c-----------------------------------------------------------------------
...
c  For simplicity, first zero out all correction function coefficients
          ...
          FG1= 0.d0
          FG2= 0.d0
C*** set up 'adiabatic' BOB parameters
          IF(NU1.GE.0) THEN
              DO  I= 0, NU1
                  U1(I)= U1A(I+1,ISTATE)
c ....   for homonuclear molecule
                  IF(IMN2.EQ.IMN1) U2(I)= U1(I)
                  ENDDO
              U1INF= U1A(NU1+2,ISTATE)
              IF(IMN2.EQ.IMN1) U2INF= U1INF
              ENDIF
          IF((NU2.GE.0).AND.(IMN2.NE.IMN1)) THEN
              DO  I= 0, NU2
                  U2(I)= U2A(I+1,ISTATE)
                  ENDDO
              U2INF= U2A(NU2+2,ISTATE)
              ENDIF
C*** set up centrifugal BOB parameters
          IF(NT1.GE.0) THEN
              DO  I= 0, NT1
                  T1(I)= T1A(I+1,ISTATE)
c ....   for homonuclear molecule
                  IF(IMN2.EQ.IMN1) T2(I)= T1(I)
                  ENDDO
               T1INF= T1A(NT1+2,ISTATE)
               IF(IMN2.EQ.IMN1) T2INF= T1INF
               ENDIF
          IF((NT2.GE.0).AND.(IMN2.NE.IMN1)) THEN
              DO  I= 0, NT2
                  T2(I)= T2A(I+1,ISTATE)
                  ENDDO
               T2INF= T2A(NT2+2,ISTATE)
               ENDIF
c=======================================================================
...
c-----------------------------------------------------------------------
     ...
     1 1,T1INF,PNA,PNA,PNA,PNA,PNA,PNA,NT1,QNA,NT1+1,(T1(I),I= 0,NT1)
              FG1= RMASS1/MASS1
              ENDIF
          IF(NT2.GE.0) THEN
...
c-----------------------------------------------------------------------
     ...
     1 2,T2INF,PNA,PNA,PNA,PNA,PNA,PNA,NT2,QNA,NT2+1,(T2(I),I= 0,NT2)
              FG2= RMASS2/MASS2
              ENDIF
          U1INF= U1INF*FC1
          U2INF= U2INF*FC2
...
c------------------------------------------------------------------------
       ...
       DATA cDS/0.468d0,0.446d0,0.423d0,0.405d0,0.39d0/
       DATA FIRST/ 1/
       SAVE FIRST, bpm, cpm
c------------------------------------------------------------------------
      IF(RHOab.LE.0) THEN
