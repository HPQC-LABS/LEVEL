c***********************************************************************
      SUBROUTINE POTGENFX(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
c     SUBROUTINE POTGEN(LNPT,NPP,IAN1,IAN2,IMN1,IMN2,VLIM,XO,RM2,VV,
c    1                                                        NCN,CNN)
c***********************************************************************
c                       Version of  6 December 2014
c** Subroutine to generate the effective adiabatic potential energy 
c  function for the electronic state specified by parameter  ISTATE
c----------------------------------------------------------------------
...
c  in the normal way with R.J. Le Roy's program LEVEL, once one inserts
c  the hard coded  value for  parameter ISTATE.  
c==============
c*** On INPUT:
c==============
...
c  to NBOB+1 terms.  ||  Allow for up to  NbMX  EMO/MLR coeffocients 
c-----------------------------------------------------------------------
      IMPLICIT NONE
      INTEGER NBOB,NbMX,ISEMX
      PARAMETER (NBOB=10,NbMX=20,ISMX=2)
     ...
     2 APSE,NVARB,NPP,LNPT,GNS,GEL, NCMM,sVSR2,LVSR,IDSTT,MM1,MMLR(9)
     ...
     5  C9adj,RM3,RET,RETsig,RETpi,RETp,RETm,BFCT2,PVSR,RH,Scalc,XXQ,
     7  REQq,RREFq,U1(0:NBOB),U2(0:NBOB),T1(0:NBOB),T2(0:NBOB),
     8  PARM(NbMX),XPARM(NbMX),rKL(NbMX,NbMX), XO(NPP),
...
ccc Define variables for the ISMX states of interest
      INTEGER ISTATE,PSEL(ISMX),PPARA(ISMX),QPARA(ISMX),APSEA(ISMX),
     1 NbetaA(ISMX),NCMMA(ISMX),MMLRA(9,ISMX),MN1RA(ISMX),MN2RA(ISMX),
     2 PADA(ISMX),QADA(ISMX),QNAA(ISMX),NU1A(ISMX),NU2A(ISMX),
     3 NT1A(ISMX),NT2A(ISMX)
      REAL*8 DSCMA(ISMX),REQA(ISMX),RrefA(ISMX),CMMA(9,ISMX),
     1 PARMA(NbMX,ISMX),XPARMA(NbMX,ISMX),U1inf(ISMX),U2inf(ISMX),
     2 T1INF(ISMX),T2inf(IDMX),U1A(0:NBOB,ISMX),U2A(0:NBOB,ISMX),
     3 T1A(0:NBOB,ISMX),T2A(0:NBOB,ISMX)
...
c*** ISTATE =1 for X(^1\Sigma^+) NaH
c           =2 for A(^1\Sigma^+) NaH 
c-----------------------------------------------------------------------
      ISTATE= 2                        %% {for example}4
c-----------------------------------------------------------------------
ccc Store parameter values for the specific ISMX states of interest
      DATA PSEL/4,3/, PPARA/6,3/,QPARA/4,4/,APSEA/-1,-1/,NbetaA/12,16/,
     1  NCMMA/3,0/,MMLRA/6,8,10,6*0, 9*0/, MN1RA/23,23/, MN2RA/1,1/,
     2  PADA/6,3/, QADA/4,3/, QNAA/ 3,3/, NU1A/-1,-1/, Nu2A/4,3/,
     3  Nt1A/-1,-1/, Nt2A/2,1/
      DATA DSCMA/15797.4d0,10041.0d0/, REQA/1.887023d0,3.1927d0/,
     3  RrefA/3.0d0,3.0d0/,
c** specify the C_m long-range coefficients for the ISMX states
     4  CMMA/3.57502D+05,5.41796D+06,1.12920D+8,6*0.d0, 9*0.d0/,
c** now specify up to 20 exponent parameters \beta_i for the ISMX states
     7 PARMA/6.582161D-02,-4.1671619D+00,-5.775325D+00,-6.25871D+00,
     8   -6.34291D+00,-6.2056D+00,-5.0814D+00,-2.205D+00,-1.678D+00,
     9  -8.36D+00,-1.493D+01,-1.118D+01,-3.1D+00,7*0.d0/,
c... now for state-2 ...
     a  3.620684D-01,1.31948D-01,1.55548D-01,8.981D-02,4.1019D-01,
     b  -2.531D-01,-2.8482D+00,-3.95D-02,1.3606D+01,2.618D+00,
     c  -3.917D+01,-8.34D+00,6.676D+01,1.08D+01,-6.17D+01,-5.D+00,
     d  2.4D+01,3*0.d0/,
c... now up to 20 (dummy) values of XPARMX for each of the ISMX states
     e  XPARMA/20*1.d0, 20*1.d0/
c** and Finally the adiabatic BOB parameters
     f U1inf/0.d0,0.d0/,U2inf/0.d0,0.d0/,T1inf/0.d0,0.d0/,
     g T2inf/0.d0,0.d0/,
c... and up to 11 ecpansion parameters for each of the ISMX states
     h U1A/11*0.0, 11*0.d0/,
     i U2A/0.0D+0,17.36D+0,28.2D+0,69.2D+0,-220.D+0,6*0.d0, -2.26D+0,
     j    -8.D+0,2.14D+1,7.D+1,7*0.d0/
     k  T1A/11*0.0, 11*0.d0/,T2A/0.d0,0.00005d0,0.003d0,8*0.d0, 
     l    0.d0,1.6d-04,9*0.d0/ 
c** and now use 'general' potgen structure - replacing READs with copys
      LNPT= 1
c
      IF(LNPT.GT.0) THEN
c** Parameter definitions listed preceeding CALL in subroutine PREPOT
c-----------------------------------------------------------------------
ccc       READ(5,*) IPOTL, PPAR, QPAR, APSE, Nbeta, IBOB
...
c-----------------------------------------------------------------------
          IF(NVARB.GT.0) THEN
ccc           IF((IPOTL.EQ.4).AND.(APSE.GT.0)) THEN
...
c*** for 'ordinary' NCMM-term MLR uLR(r) ...  with damping [if rhoAB > 0]
                  IF(rhoAB.GT.0.d0) THEN
                      CALL dampF(REQ,rhoAB,NCMM,MMLR,sVSR2,IDSTT,DM,DMP,
     1                                                           DMPP)
                      ENDIF
                  DO  J= 1,NCMM
...
c=======================================================================
          IF((sVSR2.LT.-4).OR.(sVSR2.GT.0)) THEN
          ...
          DO m= 1, NCMM
              ...
              ZK= MM-1.d0
              DM(m)= YP**(MM-1)
c... Actually ...  DM(m)= YP**(MM + sVSR2/2)  :  set it up this way to 
c   avoid taking exponential of a logarithm for fractional powers (slow)
              ...
              IF(sVSR2.EQ.0) THEN
                  ...
                  DM(m)= DM(m)*YP
                  ENDIF
              TK= bpm(MM,sVSR2) + 2.d0*cpm(MM,sVSR2)*br
