c***********************************************************************
      SUBROUTINE POTGENBr2AX(ISTATE,IMN1,IMN2,NPP,VLIM,XO,RM2,VV)
c***********************************************************************
c     SUBROUTINE POTGEN(LNPT,NPP,IAN1,IAN2,IMN1,IMN2,VLIM,XO,RM2,VV,
c    1                                                        NCN,CNN)
c-----------------------------------------------------------------------
c         **********    Version of  21 September 2012    **********
c** Subroutine to generate the effective adiabatic potential energy 
c  function for either of two electronic states of Br2 and any one of 
c  the three Br2 isotopologues using the MLR potential functions:
c  of Yukiya et al.[J.XXXxx (2012 )
c      for the   X(^1\Sigma_g^+) state  [case  ISTATE= 1]
c  and for the   A(^1\Pi_{1u}) state  [case  ISTATE= 2]
c-----------------------------------------------------------------------
...
c  line 75           as ISTATE=1  for the X(^1\Sigma_g^+) state 
c  or at line 76     as ISTATE=2 for the A(^3\Pi_{1u}) state.
c==============
c*** On INPUT:
c==============
...
c** Define arrays for storing parameter for one or multiple states
      INTEGER ISTATE,PPARA(4),QPARA(4),NLRA(4),MMLRA(4,4),NU1A(4),
     1    NT1A(4)
      REAL*8 DSCMA(4),REQA(4),RrefA(4),CMMA(4,4),PARMA(20,4),
     1    U1A(10,4),T1A(10,4)
...
c     ISTATE= 1 
c     ISTATE= 2 
ccc Specify parameter values for the two Br2 states of interest
c*** ISTATE =1 for X(^1\Sigma_g^+),  =2 for A(^3\Pi_{1u}) Br2,
      DATA PPARA/5,5,0,0/, QPARA/6,4,0,0/,MMLRA/5,6,8,0, 5,6,8,0, 8*0/,
     1  NLRA/11,13,0,0/, NU1A/0,0,0,0/, NT1A/ -1,9,0,0/ 
      DATA DSCMA/16056.875d0, 2147.864d0, 0.d0, 0.d0/,
     1  REQA/2.2810272d0,2.70432d0, 0.d0, 0.d0/,
     2  RrefA/3.12d0,3.30d0, 0.d0,0.d0/,
c** specify  up to 4 C_m long-range coefficients for up to 4 states
     4  CMMA/-3.3d2,6.274d5,1.55d7,0.d0,3.2d4,6.37d5,1.55d7,0.d0,
     5      8*0.d0/,
c** now specify 20 exponent parameters \beta_i for up to 4 states 
     6  PARMA/1.04182773D+00,8.61027D-01,1.243753D+00,1.660D-01,
     7    -1.48547D+00,-1.9263D+00,5.5190D-01,3.8080D+00,2.9730D+00,
     8    -1.5230D+00,-3.3800D+00,-1.4300D+00,8*0.D0,
     9   1.1554706D+00,5.96137D-01,-3.17462D+00,-3.536350D+00,
     a     1.062466D+01,1.44571D+01,-4.3384D+01,-4.8091D+01,1.50574D+02,
     b     6.857D+01,-3.297D+02,5.29D+01,3.064D+02,-1.81D+02,6*0.D0,
     c     40*0.D0/
c** Specify up to 10 'adiabatic' BOB parameters for up to 4 states
      DATA U1A/0.371D0,9*0.D0, 0.050D0,9*0.D0, 20*0.D0/
c** Specify up to 10 centrifugal BOB parameters for up to 4 stat0es`
      DATA T1A/10*0.D0, 0.D0,1.13D-03,2.945D-02,-1.809D-01,-5.71D-01,
     1        8.22D+00,-2.934D+01,4.99D+01,-4.19D+01,1.4D+01, 20*0.d0/
      LNPT= 1
      IAN1= 35
      IAN2= 35
      IPOTL= 4
      ...
      IDSTT= 1
      NCMM= 3
c*** Note that rhoAB is the same for the X and A states of Br2
      rhoAB= 0.912d0
      DSCM= DSCMA(ISTATE)
      REQ= REQA(ISTATE)
      Rref= RrefA(ISTATE)
